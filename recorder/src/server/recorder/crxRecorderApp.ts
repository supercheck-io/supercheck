...
import { EmbeddedRecorderWindow } from './embeddedRecorderWindow';
...
export class CrxRecorderApp extends EventEmitter implements IRecorderApp {
  ...
  async open(options?: channels.CrxApplicationShowRecorderParams) {
    ...
    // Select window type: embedded (iframe in page), sidepanel (Chrome side panel), or popup
    if (options?.window?.type === 'embedded' && options.window.tabId)
      this._window = new EmbeddedRecorderWindow(options.window.url, options.window.tabId);
    else if (options?.window?.type === 'sidepanel')
      this._window = new SidepanelRecorderWindow(options.window.url);
    else
      this._window = new PopupRecorderWindow(options?.window?.url);
