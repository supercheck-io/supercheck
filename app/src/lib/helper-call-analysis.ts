 ...
 * shared with worker/src/common/utils/helper-call-analysis.ts.
...
type HelperName = "getFile" | "readFile";
...
function recurseChildren(
  ...
  const base = (walk.base as Record<
    string,
    (node: any, state: Scope, callback: (child: any, state?: Scope) => void) => void
  >)[node.type];
...
// ─── Literal key extraction (shared with worker) ────────────────────
...
// ─── AST-based collection (shared with worker) ──────────────────────
...
// ─── Regex fallback (shared with worker) ─────────────────────────────
...
// ─── Collection API (shared with worker) ─────────────────────────────

function collectUnboundHelperCalls(
  script: string,
...
// ─── App-specific public API ─────────────────────────────────────────

/**
 * Returns true when `script` contains at least one call to `helperName`
 * that is NOT shadowed by a local declaration (function, variable, import).
 */
export function hasUnboundHelperCall(
  script: string,
  helperName: string
): boolean {
  return collectUnboundHelperCalls(script, [helperName as HelperName]).length > 0;
}
