...
describe('Secret Crypto', () => {
  ...
  describe('decryptSecret', () => {
    ...
    it('should throw for invalid envelope', () => {
      expect(() => {
        decryptSecret({ encrypted: false } as SecretEnvelope);
    ...
    it('should throw for tampered auth tag', () => {
      ...
      const decoded = JSON.parse(Buffer.from(envelope.payload, 'base64').toString());
      const tagBytes = Buffer.from(decoded.tag, 'base64');
      tagBytes[0] = tagBytes[0] ^ 0xff;
      decoded.tag = tagBytes.toString('base64');
      envelope.payload = Buffer.from(JSON.stringify(decoded)).toString('base64');
  ...
  describe('isSecretEnvelope', () => {
    ...
    it('should return false for wrong version', () => {
      expect(isSecretEnvelope({ encrypted: true, version: 2, payload: 'test' })).toBe(false);
    });

    it('should return false for encrypted=false', () => {
      expect(isSecretEnvelope({ encrypted: false, version: 1, payload: 'test' })).toBe(false);
    });
  });
