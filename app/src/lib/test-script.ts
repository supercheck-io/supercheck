/**
 * Decode, sanitize, and persist Playwright/k6 scripts stored on tests.
 *
 ...
 * sourcemap. Execution paths must accept all three and never feed binary
 * blobs to Playwright.
 */
...
export function decodeStoredTestScript(script: string | undefined | null): string {
  if (!script) {
    return "";
  }
...
export function encodeStoredTestScript(script: string): string {
  return Buffer.from(sanitizeTestScript(decodeStoredTestScript(script)), "utf8").toString(
    "base64",
  );
}
