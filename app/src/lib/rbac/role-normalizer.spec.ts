...
describe('Role Normalizer', () => {
  ...
  describe('getHigherRole', () => {
    it('should return SUPER_ADMIN when comparing with any role', () => {
      expect(getHigherRole(Role.SUPER_ADMIN, Role.ORG_OWNER)).toBe(Role.SUPER_ADMIN);
      expect(getHigherRole(Role.PROJECT_VIEWER, Role.SUPER_ADMIN)).toBe(Role.SUPER_ADMIN);
    });

    it('should return ORG_OWNER when comparing ORG_OWNER and ORG_ADMIN', () => {
      expect(getHigherRole(Role.ORG_OWNER, Role.ORG_ADMIN)).toBe(Role.ORG_OWNER);
      expect(getHigherRole(Role.ORG_ADMIN, Role.ORG_OWNER)).toBe(Role.ORG_OWNER);
    });

    it('should return first role when both are equal', () => {
      expect(getHigherRole(Role.PROJECT_EDITOR, Role.PROJECT_EDITOR)).toBe(Role.PROJECT_EDITOR);
    });

    it('should handle all combinations correctly', () => {
      expect(getHigherRole(Role.PROJECT_ADMIN, Role.PROJECT_EDITOR)).toBe(Role.PROJECT_ADMIN);
      expect(getHigherRole(Role.PROJECT_EDITOR, Role.PROJECT_VIEWER)).toBe(Role.PROJECT_EDITOR);
    });
  });

  describe('normalizeRoles', () => {
    it('should normalize an array of role strings', () => {
      const roles = normalizeRoles(['super_admin', 'org_owner', 'project_viewer']);
      expect(roles).toEqual([Role.SUPER_ADMIN, Role.ORG_OWNER, Role.PROJECT_VIEWER]);
    });

    it('should handle mixed input types', () => {
      const roles = normalizeRoles([Role.SUPER_ADMIN, 'org_owner', null, undefined]);
  ...
  describe('findHighestRole', () => {
    it('should find SUPER_ADMIN as highest in mixed array', () => {
      const highest = findHighestRole(['project_viewer', 'super_admin', 'org_owner']);
      expect(highest).toBe(Role.SUPER_ADMIN);
    });

    it('should find ORG_OWNER when no SUPER_ADMIN present', () => {
      const highest = findHighestRole(['project_viewer', 'org_owner', 'org_admin']);
  ...
  describe('Edge Cases and Boundary Conditions', () => {
    ...
    it('should handle boolean input', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation();
      expect(normalizeRole(true as unknown as string)).toBe(Role.PROJECT_VIEWER);
      expect(normalizeRole(false as unknown as string)).toBe(Role.PROJECT_VIEWER);
      warnSpy.mockRestore();
    });
    ...
    it('should handle object input', () => {
      ...
      expect(normalizeRole({ role: 'admin' } as unknown as string)).toBe(Role.PROJECT_VIEWER);
      warnSpy.mockRestore();
    });

    it('should handle array input', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation();
      expect(normalizeRole(['admin'] as unknown as string)).toBe(Role.PROJECT_VIEWER);
      warnSpy.mockRestore();
    });
    ...
    it('should handle SQL injection attempts in role strings', () => {
      const warnSpy = jest.spyOn(console, 'warn').mockImplementation();
      expect(normalizeRole("'; DROP TABLE users; --")).toBe(Role.PROJECT_VIEWER);
