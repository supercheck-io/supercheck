...
} from "./investigation-progress-card";
...
describe("SRE investigation progress card", () => {
  it("summarizes agent step events without exposing raw payloads", () => {
    ...
    expect(summary).toEqual({
      kind: "step",
      title: "Agent step 2",
      description: "1 read-only tool call",
      status: "running",
      elapsedMs: 1250,
      tools: [
        {
  ...
  it("falls back to safe tool labels for unexpected names", () => {
    const summary = summarizeSreAgentProgressEvent("agent.step", {
      event: {
        toolCalls: [
          {
            toolCallId: "call-2",
            toolName: "unsafe tool with spaces",
            input: { token: "secret-value" },
          },
        ],
      },
    ...
    expect(summary?.tools).toEqual([
      { id: "call-2", name: "read-only tool", status: "called" },
    ]);
  });
  ...
  it("renders recent progress events", () => {
    ...
    expect(
      screen.getByText(
        "Evidence ev-monitor-timeout: Monitor timeout (event/native)",
      ),
    ).toBeInTheDocument();
