...
export function PrivateAgentFormDialog({
  open,
  onOpenChange,
  onSaved,
}: PrivateAgentFormDialogProps) {
  ...
  const [registeredAgentId, setRegisteredAgentId] = useState<string | null>(
    null,
  );
  ...
  const copyAgentId = async () => {
    ...
    await navigator.clipboard.writeText(registeredAgentId);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-4xl min-w-2xl gap-3 overflow-y-auto p-5">
        <DialogHeader>
          ...
          <DialogDescription>
            Private Agents proxy read-only connector requests to private
            networks. The token is shown once.
          </DialogDescription>
        </DialogHeader>
        ...
        {registrationToken ? (
          <div className="space-y-4">
            <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200">
              Copy this token now. It is stored only as a hash and cannot be
              recovered later.
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="private-agent-id">Agent ID</Label>
              <div className="flex gap-2">
                ...
                />
                <Button type="button" variant="outline" onClick={copyAgentId}>
            ...
            <div className="rounded-lg border bg-muted/30 p-3 text-sm text-muted-foreground">
              Start the agent with this token in its environment. Connector
              execution remains read-only and policy-gated by Supercheck.
            </div>
            <DialogFooter>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-lg border p-3 md:col-span-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    setProjectScoped(checked === true)
                  }
                />
                <span className="space-y-1 text-sm">
                  <span className="block font-medium">
                    Scope this agent to the current project
                  </span>
                  <span className="block text-muted-foreground">
                    Recommended. Organization-wide agents can serve all projects
                    in the organization.
                  </span>
                </span>
              </label>
