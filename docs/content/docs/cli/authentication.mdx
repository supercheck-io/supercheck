...
The CLI automatically handles rate limiting with exponential backoff. When the API returns `429`, the CLI reads the `Retry-After` header (if present) and waits before retrying.
