...
**Via API:**
```bash
curl -X POST https://demo.supercheck.dev/api/cli-tokens \
...
curl -X POST https://demo.supercheck.dev/api/jobs/<id>/trigger \
...
| `SUPERCHECK_URL` | No | Base URL (default: `https://demo.supercheck.dev`) |
