...
|----------|----------|-------------|
| `SUPERCHECK_TOKEN` | Usually | CLI token for most commands and for `job trigger --wait` |
| `SUPERCHECK_URL` | No | Base URL (default: `https://app.supercheck.io`) |
| `SUPERCHECK_ORG` | No | Default organization ID (UUID) |
| `SUPERCHECK_PROJECT` | No | Default project ID (UUID) |
| `SUPERCHECK_TRIGGER_KEY` | For `job trigger` | Job-specific trigger key |
...
<Callout type="info">
  For CI/CD, use `SUPERCHECK_TRIGGER_KEY` with `supercheck job trigger`. Add `SUPERCHECK_TOKEN` only when you also use `--wait` or run other API-backed commands such as `deploy`, `diff`, or `whoami`.
</Callout>
...
Job trigger keys use their server-side per-key configuration. The default is 100 requests per 60-second window, but administrators can configure a different limit for an individual key. Other endpoints may apply their own operation-specific limits.

The CLI automatically handles rate limiting with exponential backoff. When the API returns `429`, the CLI reads the `Retry-After` header (if present) and waits before retrying an idempotent request.
