...
For streaming commands, global `--json` produces newline-delimited JSON (NDJSON), one event per line.
...
| `--type` | No | Job runner type: `playwright` or `k6` |
...
| `--type` | Filter by local test type (`browser`, `performance`) |
...
| `--payload` | No | Provider configuration as a JSON object |
| `--data` | No | Alias for `--payload` |
| `--config` | No | Legacy alias for `--payload` |
...
| `--name` | Provider name |
| `--type` | Provider type |
| `--payload` | JSON configuration object (replaces the entire config; include all fields) |
| `--data` | Alias for `--payload` |
| `--config` | Legacy alias for `--payload` |
...
| `--payload` | Yes* | Provider configuration as a JSON object |
| `--data` | Yes* | Alias for `--payload` |
| `--config` | Yes* | Legacy alias for `--payload` |
...
`--force` skips the confirmation prompt. Test scripts are stored as source text, not precompiled output.
