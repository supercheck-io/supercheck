...
> Note: Reserved for future use. Defined in the configuration schema but not currently applied during local CLI execution.

| Field | Type | Description |
|-------|------|-------------|
| `runLocation` | `string` | Default execution location code (e.g. `us-east`) |
...
| `browser` | `string` | Browser to use (default: `chromium`; local runner executes with chromium) |
...
|-------|------|-------------|
| `id` | `string` | Database UUID (present for existing resources, omit for new) |
| `name` | `string` | Status page display name |
| `subdomain` | `string` | Status page subdomain identifier |
| `status` | `string` | Status page state (`draft`, `published`, `archived`) |
| `headline` | `string` | Public page headline |
| `description` | `string` | Public description text |
| `supportUrl` | `string` | External support URL |
| `language` | `string` | ISO language code (e.g. `en`) |
| `components` | `object[]` | List of status page components (`name`, `description`, `monitors`, `position`) |
| `branding` | `object` | Page styling options (`bodyBackgroundColor`, `fontColor`, `greens`, `reds`) |
| `notifications` | `object` | Subscription toggles (`allowEmailSubscribers`, `allowWebhookSubscribers`, `allowRssFeed`) |
...
| `config` | `object` | Provider-specific configuration, including webhook `method`, `headers`, and `bodyTemplate` |

`pull` omits masked provider secrets such as webhook URLs and credentials. You can still edit non-secret settings such as `bodyTemplate`; `diff` detects those changes without exposing stored secrets.
...
`diff` includes notification provider settings such as webhook `bodyTemplate`. Deploy removes generated sourcemap references and unexpected binary suffixes, then stores the UTF-8 test source using the API's Base64 transport format. `--force` only skips confirmation; it does not change script processing.
