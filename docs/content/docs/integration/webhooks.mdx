  ...
  switch (event) {
    case 'job.completed':
      if (data.result === 'passed') {
        console.log(`✓ Job ${data.job_name} passed`);
        // Trigger deployment, send notification, etc.
      } else {
        console.log(`✗ Job ${data.job_name} failed`);
        // Alert team, create incident, etc.
      }
      break;

    case 'monitor.down':
      console.log(`⚠ Monitor ${data.monitor_name} is down`);
      // Create PagerDuty incident, send alert, etc.
      break;

    case 'monitor.recovered':
      console.log(`✓ Monitor ${data.monitor_name} recovered`);
...
def handle_webhook():
    ...
    if event == 'job.completed':
        if data['result'] == 'passed':
            print(f"✓ Job {data['job_name']} passed")
            # Trigger deployment
        else:
            print(f"✗ Job {data['job_name']} failed")
            # Alert team

    elif event == 'monitor.down':
        print(f"⚠ Monitor {data['monitor_name']} is down")
        # Create incident

    elif event == 'monitor.recovered':
        print(f"✓ Monitor {data['monitor_name']} recovered")
        # Resolve incident
