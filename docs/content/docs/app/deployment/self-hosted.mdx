...
- `WORKER_REPLICAS`: Controls the number of worker containers. **Worker-side setting.** Set in Docker Compose environment.
- `RUNNING_CAPACITY`: Maximum concurrent test runs in running state. **App-side setting.** Set equal to total worker replicas.
- `QUEUED_CAPACITY`: Maximum queued test runs before rejecting submissions. **App-side setting.** Set on the app server.
</Callout>
