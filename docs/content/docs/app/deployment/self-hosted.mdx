  ...
  <Tab value="Production (HTTPS)">
    <Steps>
      ...
      <Step>
        ...
        <Callout type="info">
        `STATUS_PAGE_DOMAIN` is the reserved namespace for default status page URLs (`[uuid].STATUS_PAGE_DOMAIN`). In the HTTPS Compose examples, Supercheck derives the custom-domain target shown in the UI from that value, typically as `cname.STATUS_PAGE_DOMAIN`.
        </Callout>

        <Callout type="warn">
        Set `STATUS_PAGE_DOMAIN` to a publicly reachable hostname. The wildcard `*` DNS record from the previous step usually covers the derived target (for example `cname.yourdomain.com`) automatically, so you should not add a second A/AAAA record on the customer's custom hostname.
        </Callout>
      </Step>
