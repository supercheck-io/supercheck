...
Workers execute Playwright and k6 workloads in isolated Docker containers.

<Callout type="info">
Single-location deployments should keep `WORKER_LOCATION=local` (the default). This makes each worker process all regional queues on that server.
</Callout>

<Callout type="info">
**Where each setting lives:**
- `WORKER_REPLICAS` — controls how many worker containers Docker starts. **Docker Compose / worker side.**
- `RUNNING_CAPACITY` — controls the maximum number of test runs the App allows into `running` state simultaneously. **App-side setting only.** Set it equal to the total number of workers so the queue gate matches actual execution throughput.
- `QUEUED_CAPACITY` — controls the maximum number of queued runs the App accepts before returning capacity-limit errors. **App-side setting only.**
</Callout>
