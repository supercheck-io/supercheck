...
| Worker logs `Failed query` for every database call | Set `SELF_HOSTED=true` on remote workers that connect to the bundled self-hosted Postgres so the worker does not require PostgreSQL SSL |
