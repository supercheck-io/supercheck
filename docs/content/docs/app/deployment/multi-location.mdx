...
`RUNNING_CAPACITY` is an **App-side** setting — the App uses it as a gate to control how many test runs can be in `running` state simultaneously. It is **not** a worker setting and does not need to be set on remote worker hosts.

`WORKER_REPLICAS` controls how many worker containers start on a given host.

For multi-host deployments, set `RUNNING_CAPACITY` on the **main server (App)** equal to the **total number of workers across all hosts**:

```bash
# Main server: total of 3 workers across all hosts (1 + 1 + 1)
RUNNING_CAPACITY=3 docker compose up -d
