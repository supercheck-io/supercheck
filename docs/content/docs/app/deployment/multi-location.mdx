...
<Steps>
  ...
  <Step>
    ...
    **3. Create `.env`:**
    ...
    <Callout type="warn">
    **Important**: The worker connects to Redis using `REDIS_HOST`, `REDIS_PORT`, and `REDIS_PASSWORD` — not `REDIS_URL`. Make sure all three are set correctly.
    </Callout>
...
Use `RUNNING_CAPACITY` as the source-of-truth concurrency setting. Keep it equal to `WORKER_REPLICAS` in the same deployment so configured capacity matches available worker replicas.
...
<Callout type="warn">
The worker uses `REDIS_HOST`, `REDIS_PORT`, and `REDIS_PASSWORD` environment variables — **not** `REDIS_URL`. If the worker logs show it connecting to `127.0.0.1` or `localhost`, check that `REDIS_HOST` is set to your main server&apos;s IP address.
</Callout>
