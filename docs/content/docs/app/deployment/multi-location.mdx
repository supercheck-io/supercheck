...
|-------|-------------------|
| Worker can't reach DB | `docker compose -f docker-compose-worker.yml exec worker curl -sf http://127.0.0.1:8000/health/ready` and `docker run --rm postgres:18 psql "$DATABASE_URL" -c "SELECT 1"` |
