...
sequenceDiagram
    ...
    App->>Redis: Enqueue monitor or test job
    Remote->>Redis: Consume location queue
    Remote->>PG: Read monitor or test definition
    Remote->>Remote: Execute check
    Remote->>PG: Write result
```

A worker can appear in Super Admin (heartbeat over Redis) and still fail monitor jobs if PostgreSQL is unreachable. Redis connectivity is not enough.
...
<Callout type="info">
**Routing rules**: Playwright jobs go to `playwright-global` (any worker picks them up). Each K6 job targets one resolved regional queue, or `k6-global` when any-location execution is allowed. Monitors run only on their configured location queues.
</Callout>
...
<Callout type="info">
Offline locations stay visible in the UI to prevent transient outages from silently removing regions from monitor configs. Monitors degrade gracefully to the online subset. New K6 submissions require a live worker advertising the resolved queue. Work already queued can wait for a matching worker to return.
</Callout>

Keep at least one worker running in each region where you offer monitoring. Monitor scheduling requires a live regional heartbeat and does not enqueue work to wake a region with zero workers. A region code labels the worker; it does not move execution geographically. Use a specific `WORKER_LOCATION` for each real region, since `local` consumes all regional queues.
...
|-------|-------------------|
| Worker can't reach DB | Check readiness with `docker compose -f docker-compose-worker.yml exec worker curl -sf http://127.0.0.1:8000/health/ready`, then test the database directly with `docker run --rm postgres:18 psql "postgresql://USER:PASSWORD@MAIN_SERVER_IP:5432/supercheck" -c "SELECT 1"` |
...
| Worker visible in Super Admin but monitors do not run | Heartbeat uses Redis. Monitor jobs also need PostgreSQL. Confirm `DATABASE_URL` points at the **same** database as the app, `SELF_HOSTED=true` for Docker Postgres without TLS, and ports 5432/6379/9000 are reachable. |
| Worker can't reach Redis | `docker run --rm redis:8 redis-cli -h MAIN_SERVER_IP -a PASSWORD ping` |
| Verify worker location | `docker compose logs worker \| grep "initialized for location"` |
