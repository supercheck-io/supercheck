...
- `WORKER_REPLICAS`: Controls the number of worker containers per server. **Worker-side setting.** Set individually on each worker server.
- `RUNNING_CAPACITY`: Maximum concurrent test runs in running state. **App-side setting.** Set on main server to total workers across all servers.
- `QUEUED_CAPACITY`: Maximum queued test runs before rejecting submissions. **App-side setting.** Set on the main server.
</Callout>
