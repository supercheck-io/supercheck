...
<Steps>
  ...
  <Step>
    ...
    **CNAME Target:**
    ...
    - **Self-hosted:** Point to the full configured `STATUS_PAGE_DOMAIN` value (for example, `example.com` or `supercheck.example.com`)
    </Callout>
...
| 404 on custom domain | Ensure the configured custom domain is not `STATUS_PAGE_DOMAIN` (or one of its subdomains). Also verify the CNAME record points to the full configured `STATUS_PAGE_DOMAIN` value (for example, `example.com` or `supercheck.example.com`). |
...
<Callout type="info">
**Self-hosted configuration:** Set `STATUS_PAGE_DOMAIN` to the full hostname you want to reserve for default status page URLs and CNAME targets (for example, `example.com` or `supercheck.example.com`). URL-like values such as `https://supercheck.example.com:443` are normalized automatically, but plain hostnames are recommended.
</Callout>
