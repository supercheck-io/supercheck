...
| **Self-hosted (Docker Compose)** | Traefik provisions Let's Encrypt certificates for configured routes. Ensure your reverse proxy is configured to accept traffic for your custom domain. |
| **Behind Cloudflare proxy** | Cloudflare handles SSL at the edge. Set SSL/TLS mode to **Full** or **Full (Strict)** in your Cloudflare dashboard. |
| **Other reverse proxies** | Configure TLS termination in your reverse proxy (nginx, Caddy, etc.) |
...
| HTTPS not working (self-hosted) | Ensure your reverse proxy is configured to accept traffic for your custom domain and handle TLS |
