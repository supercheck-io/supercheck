...
flowchart LR
    ...
    style A fill:#fffbeb,stroke:#b45309,color:#92400e
    style B fill:#fff7ed,stroke:#c2410c,color:#9a3412
    style C fill:#eff6ff,stroke:#1e40af,color:#1e40af
    style D fill:#ecfdf5,stroke:#047857,color:#065f46
    style S fill:#eef2ff,stroke:#4338ca,color:#3730a3
```
...
sequenceDiagram
    participant Team as Responder
    participant Page as Status Page
    participant Email as Email (SMTP)
    participant Slack as Slack
    participant Hook as Webhook
    participant Feed as RSS · iCal · Badge

    Team->>Page: Create, update, or resolve an incident
    Page->>Page: Update component status and timeline
    opt Notify subscribers enabled
        Page->>Email: Incident notification email
        Page->>Slack: Channel message (color-coded by impact)
        Page->>Hook: JSON incident event
    end
    Page->>Feed: Refresh RSS, iCal, and status badge
```
...
- **Page Subscriptions** — Allow visitors to subscribe to all status updates
- **Email, Webhook, and Slack Notifications** — Enable each supported subscriber channel independently
- **Incident Subscriptions** — Allow subscriptions to individual incidents
...
```mermaid
flowchart LR
    A["1. Configure Domain"] --> B["2. Add CNAME Record"] --> C["3. Verify DNS"] --> D["4. Active"]
    style A fill:#eef2ff,stroke:#4338ca,color:#3730a3
    style B fill:#eff6ff,stroke:#1e40af,color:#1e40af
    style C fill:#fffbeb,stroke:#b45309,color:#92400e
    style D fill:#ecfdf5,stroke:#047857,color:#065f46
```
