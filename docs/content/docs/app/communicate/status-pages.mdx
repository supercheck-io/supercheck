...
<Steps>
  <Step>
    **Configure in Supercheck**
    ...
    2. Enter a subdomain you control (e.g., `status.yourcompany.com`)
    3. Click **Save**
  </Step>
  <Step>
    **Add DNS Record**
    ...
    | Type | Name | Target |
    |------|------|--------|
    | CNAME | `status.yourcompany.com` | `supercheck.io` |

    Use the full hostname unless your DNS provider explicitly asks for a label relative to your zone. The **Target** is shown in the Settings tab after you save your domain.

    <Callout type="info">
    **CNAME Target:**
    - **Cloud users:** Point to `supercheck.io`
    - **Self-hosted:** Point to the hostname shown in **Settings**. This is typically `STATUS_PAGE_DOMAIN`, or `APP_DOMAIN` / `APP_URL` when `STATUS_PAGE_DOMAIN` is not set.
    </Callout>
    ...
    **Reserved domain namespace:** Do not set your custom domain to `STATUS_PAGE_DOMAIN` itself or any of its subdomains. That namespace is reserved for default UUID URLs (`[uuid].STATUS_PAGE_DOMAIN`). Use a separate hostname and point its CNAME to the target shown in Settings.
    ...
    <Callout type="warn">
    **Use a subdomain:** Root/apex domains may use DNS flattening and can fail CNAME-based verification. Use a subdomain such as `status.yourcompany.com` for the most reliable setup.
    </Callout>
...
| Verification fails on an apex/root domain | Use a subdomain such as `status.example.com` instead of the zone apex |
...
<Callout type="info">
**Self-hosted configuration:** Set `STATUS_PAGE_DOMAIN` to the base hostname used for default UUID status page URLs (for example `example.com`). If omitted, Supercheck falls back to `APP_DOMAIN` and then `APP_URL`. URL-like values such as `https://example.com:443` are normalized automatically, but plain hostnames are recommended.
</Callout>
