...
<Steps>
  ...
  <Step>
    **Add DNS Record**
    ...
    |------|----------|--------|
    | CNAME | `status.yourcompany.com` | `target shown in Settings` |

    The Settings tab shows the full hostname and the required CNAME target. Some DNS providers expect the full hostname, while others expect only the label relative to your zone. Use the same target either way.

    <Callout type="info">
    **CNAME Target:**
    - **Cloud users:** Point to the hostname shown in Settings.
    - **Self-hosted Docker/Kubernetes examples:** The UI typically shows a target derived from `STATUS_PAGE_DOMAIN`, usually `cname.STATUS_PAGE_DOMAIN`.
    - **Advanced/self-managed deployments:** Always use the exact target shown in Settings.
    </Callout>
    ...
    **Reserved domain namespace:** Do not set your custom domain to `STATUS_PAGE_DOMAIN` itself or any of its subdomains. That namespace is reserved for default UUID URLs (`[uuid].STATUS_PAGE_DOMAIN`). Use a separate hostname and point its CNAME to the target shown in Settings.
    ...
    <Callout type="info">
    **Do not add both record types on the same hostname:** Your customer-facing hostname should be a CNAME (or your provider's equivalent ALIAS/ANAME). The target hostname shown in Settings may itself resolve through A/AAAA or wildcard DNS records, and that is normal.
    </Callout>

    <Callout type="info">
    **Target hostname must already be live:** Verification checks that the hostname shown in Settings is publicly resolvable before activating the custom domain. Do not add A/AAAA records on your customer-facing hostname to satisfy that check. Instead, make sure the target hostname itself is already backed by your ingress, proxy, wildcard DNS, or other origin DNS setup.
    </Callout>
...
| "Target hostname is not publicly resolvable yet" | Keep your customer-facing hostname as a CNAME only. The target shown in Settings must already resolve publicly, often via A/AAAA or wildcard DNS records on the target side. |
...
<Callout type="info">
**Self-hosted configuration:** Set `STATUS_PAGE_DOMAIN` to the full hostname you want to reserve for default status page URLs (`[uuid].STATUS_PAGE_DOMAIN`). Supercheck derives the primary custom-domain target from it automatically and, in the Docker/Kubernetes examples, that is usually `cname.STATUS_PAGE_DOMAIN`. If `STATUS_PAGE_DOMAIN` is already the ingress hostname, the UI will reuse it directly. CNAME verification accepts the primary derived target plus `STATUS_PAGE_DOMAIN`, `cname.STATUS_PAGE_DOMAIN`, and `ingress.STATUS_PAGE_DOMAIN` as valid targets. URL-like values such as `https://supercheck.example.com:443` are normalized automatically, but plain hostnames are recommended.
</Callout>

<Callout type="info">
**DNS provider input formats vary:** Some DNS providers want the full hostname (`status.example.co.uk`), while others want only the label relative to your zone (`status`). The required CNAME target stays the same in either format.
</Callout>
