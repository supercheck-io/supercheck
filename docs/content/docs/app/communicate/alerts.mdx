...
sequenceDiagram
    box rgb(8,145,178)
        participant Check as Monitor, test, or job
    end
    box rgb(5,150,105)
        participant Worker as Worker
    end
    box rgb(234,88,12)
        participant Channel as Email / Slack / Telegram / Webhook
    end
    box rgb(217,119,6)
        participant OnCall as PagerDuty / Splunk On-Call
    end

    Check->>Worker: Failure or recovery detected
    Worker->>Worker: Resolve channels and smart thresholds
    Worker->>Channel: Render template variables
    Channel-->>Worker: Delivery status
    Worker->>OnCall: Trigger (CRITICAL) or resolve (RECOVERY)
    OnCall-->>Worker: Acknowledgement
    Worker->>Worker: Record alert history
```
