...
<Callout type="info">
The webhook UI supports custom JSON headers for provider API keys. Supercheck stores these headers in the encrypted provider config and masks them when the provider is loaded for editing.
</Callout>
