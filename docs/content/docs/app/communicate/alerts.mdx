...
sequenceDiagram
    participant Check as Monitor or job
    participant Worker as Worker
    participant Channel as Email, Slack, or webhook
    participant OnCall as PagerDuty or Splunk On-Call

    Check->>Worker: Failure or recovery
    Worker->>Channel: Render template variables
    Worker->>OnCall: trigger or CRITICAL / resolve or RECOVERY
...
<Card icon={<Users className="text-blue-500" />} title="Teams">
...
<div style={{maxWidth: '400px'}}>
![Discord Alert](/screenshots/alert-discord.png)
</div>
