...
| `{{victorOpsMessageType}}` | VictorOps/Splunk On-Call lifecycle value | `CRITICAL`, `RECOVERY` |
| `{{splunkOnCallMessageType}}` | Same as victorOpsMessageType | `CRITICAL`, `RECOVERY` |
...
Use a webhook body template for the [Splunk On-Call REST endpoint integration](https://help.splunk.com/en/splunk-enterprise/alert-and-respond/splunk-on-call/integrations-with-splunk-on-call/rest-endpoint-integration-for-splunk-on-call). The `message_type` field controls the incident lifecycle, so use `{{victorOpsMessageType}}` or `{{splunkOnCallMessageType}}` instead of a hard-coded value.
...
<Callout type="info">
`{{victorOpsMessageType}}` renders `CRITICAL` for failure/trigger alerts and `RECOVERY` for monitor recovery alerts. Keep `{{dedupKey}}` in `entity_id` so Splunk On-Call can correlate the recovery with the original incident.
</Callout>
...
<Callout type="warn">
For OpsGenie, you must also add an `Authorization` header with your API key. Since the webhook UI does not currently support custom headers, you can use OpsGenie's [email integration](https://docs.opsgenie.com/docs/email-integration) as an alternative, or set the API key via your API endpoint proxy.
</Callout>
