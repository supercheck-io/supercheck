...
- **Investigate → Copilot** provides read-only conversational help without cluttering incident detail pages. Slash actions remain visible, while the single `@ context` control opens incident, service, and recent-deployment references. Inline charts remain bounded, cited, and accessible; the developer preview control is not exposed in the responder composer.
