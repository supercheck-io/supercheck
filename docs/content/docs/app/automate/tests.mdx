...
sequenceDiagram
    ...
    You->>App: Run or schedule a test
    App->>Queue: Enqueue test job
    App-->>You: Live run status (SSE)
    Queue->>Worker: Dequeue job
    Worker->>Sandbox: Create sandboxed run
    Sandbox-->>Worker: Results, traces, screenshots
    Worker->>Store: Save results and artifacts
    Worker->>App: Publish completion
    App-->>You: Report, screenshots, and traces
```
