...
sequenceDiagram
    ...
    box rgb(124,58,237)
        participant Editor as Playground editor
    end
    box rgb(37,99,235)
    ...
    box rgb(13,148,136)
        participant Sandbox as K3s + gVisor job
    end

    User->>Editor: Click Run
    activate Editor
    Editor->>API: Submit script
    API->>API: Validate script and capacity
    API->>Worker: Queue run
    API-->>Editor: Run ID (SSE stream)
    deactivate Editor
    activate Worker
    Worker->>Sandbox: Execute in isolated container
    Sandbox-->>Worker: Results, screenshots, traces
    Worker->>API: Publish completion
    deactivate Worker
    API-->>Editor: Live logs, then report URL
```
