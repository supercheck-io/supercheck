...
Generate complete, working test scripts from plain English descriptions. AI Create uses your configured AI provider to understand your requirements and produce well-structured Playwright code.
 ...
 * File variables can be read with readFile() in Playwright.
 * @see /docs/app/automate/variables for setup instructions
 */
...
Variables are resolved server-side before execution. Secrets are encrypted and execution output is redacted before persistence/return. File variables are intended for reusable, non-sensitive fixtures and datasets.
