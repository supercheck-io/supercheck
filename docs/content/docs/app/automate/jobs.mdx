...
| Setting | Description |
|---------|-------------|
| **Job failures** | Notify when any test in the job fails |
| **Job success completion** | Notify when all tests pass (optional) |
| **Job timeouts** | Notify if the job exceeds its maximum execution time |
...
sequenceDiagram
    ...
    box rgb(234,88,12)
        participant Alerts as Notification channels
    end

    CI->>API: POST /trigger (trigger key)
    activate API
    API->>API: Verify trigger key and job
    API->>Worker: Queue job
    API-->>CI: 202 Accepted (run ID)
    deactivate API
    activate Worker
    Worker->>Worker: Execute tests in gVisor sandbox
    Worker->>API: Publish run result
    API->>Alerts: Send notifications
    deactivate Worker
    CI->>API: GET run status
    API-->>CI: Final result
```
...
2. Open the job's **Edit** action and continue to **CI/CD Settings**
3. Click **Create Key**

![CI/CD Tab](/screenshots/edit-job-add-cicd-apikey-1.png)

![Create API Key](/screenshots/edit-job-add-cicd-apikey-2.png)
