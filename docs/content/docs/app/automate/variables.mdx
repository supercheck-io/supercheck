...
- **Access:** `readFile(key)` returns the file contents; `getFile(key)` returns the file path (for k6’s `open()`)
...
<Callout type="info">
File variables are stored in S3/MinIO and made available inside the test execution container at runtime. Use `readFile(key)` to read the file contents directly (no `fs` import needed), or `getFile(key)` to get the path for k6’s `open()` function.
</Callout>
 ...
 * readFile(key) returns the file contents as a string directly — no fs import needed.
 * getFile(key) returns the file path, used with k6's open().
 */
