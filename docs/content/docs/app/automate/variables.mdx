...
- **Access:** Playwright can use `readFile(key)`; k6 should use `open(getFile(key))` in init context
...
<Callout type="info">
File variables are stored in S3/MinIO and made available inside the test execution container at runtime. In Playwright, use `readFile(key)` to read the contents directly. In k6, use `open(getFile(key))` during init context, and prefer `SharedArray` for datasets shared across VUs.
</Callout>
 ...
 * Playwright can read contents directly with readFile(key).
