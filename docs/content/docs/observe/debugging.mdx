...
<Steps>
  ...
  <Step>Filter by **Status: Failed**/**Error** and the time range</Step>
  <Step>Open a failed trace to investigate</Step>
</Steps>
...
4. Did earlier spans already show degraded behavior?
...
<Steps>
  ...
  <Step>Review navigation/network spans for load failures.</Step>
  <Step>Verify selector and wait conditions.</Step>
  <Step>Adjust timeouts only if required by real user behavior.</Step>
</Steps>
...
**Debugging Steps**:
<Steps>
  <Step>Check status code and response body.</Step>
  <Step>Inspect request payload/headers in span attributes.</Step>
  <Step>Confirm auth spans succeeded.</Step>
  <Step>See if other tests to same API failed (system issue vs test issue).</Step>
</Steps>
...
**Debugging Steps**:
<Steps>
  <Step>Review timeline—what else ran during the wait?</Step>
  <Step>Check pending network requests.</Step>
  <Step>Confirm selector/condition is correct and reachable.</Step>
  <Step>Spot performance regressions earlier in the run.</Step>
</Steps>
...
**Debugging Steps**:
<Steps>
  <Step>Compare expected vs actual from the span payload.</Step>
  <Step>Verify setup steps (login/data seeding) succeeded.</Step>
  <Step>Check response data for mismatches.</Step>
  <Step>Confirm timing—was data loaded before asserting?</Step>
</Steps>
