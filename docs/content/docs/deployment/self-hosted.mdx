...
<Callout type="info">
Each worker runs one Playwright test at a time with **2 parallel browser workers** inside the container. Scale workers (not Playwright workers) for more throughput.
</Callout>
