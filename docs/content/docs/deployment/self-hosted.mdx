...
**Docker Prerequisites:** This guide requires the modern Docker Compose plugin (the `docker compose` command with a space). The legacy `docker-compose` command (with hyphen) is not supported.
