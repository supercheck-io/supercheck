  ...
  <Step>
    ...
    <Tabs items={['Production (Secure)', 'Testing (Insecure)']}>
      <Tab value="Production (Secure)">
        ...
        > [!CAUTION]
        > **Security Warning**: Exposing database ports (5432, 6379, 9000) to the public internet is dangerous.
        > You **MUST** use a firewall (like `ufw` or AWS Security Groups) to whitelist ONLY the IP addresses of your worker servers.
        > Alternatively, use a VPN (WireGuard/Tailscale) to connect workers privately.
