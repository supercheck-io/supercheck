...
icon: Workflow
---
Design, schedule, and run browser, API, database, performance, and custom tests—locally, on a schedule, or from CI/CD.

<Callout type="info">
  Use Playwright for browser flows, REST/GraphQL for APIs, SQL for databases, and k6 for performance tests. Keep Playwright and k6 scripts separate for predictable runs.
</Callout>

## Overview

What you can do:
- **Mix Test Types**: Browser, API, DB, custom, and performance (k6).
- **Schedule & Retry**: Cron jobs with timezones, retries, and alerts.
- **Use Variables & Secrets**: Project-scoped values with RBAC.
- **Go Parallel Safely**: Capacity controls prevent noisy neighbors.
- **Integrate Easily**: Trigger jobs/tests from CI/CD or API.
...
<Steps>
  <Step>Go to **Automate → Create** and pick a test type.</Step>
  <Step>Author the test in the playground; validate with **Run Test**.</Step>
  <Step>Save it for reuse in jobs and monitors.</Step>
</Steps>

### Step 2: Create a Job

<Steps>
  <Step>Open **Automate → Jobs** and choose **Create Job**.</Step>
  <Step>Select tests (mix types if needed) and set timeouts/retries.</Step>
  <Step>Save and run once to verify the combined report.</Step>
</Steps>

### Step 3: Schedule Automatic Execution

<Steps>
  <Step>Edit the job and enable scheduling (cron with timezone).</Step>
  <Step>Attach alerts (Slack/email/webhook) for failures and recoveries.</Step>
  <Step>Save—your job now runs hands-free.</Step>
</Steps>
...
### Capacity Limits
- **Running Capacity**: Max concurrent executions (default: 5)
- **Queued Capacity**: Max queued submissions (default: 50)
...
<Callout type="warning">
  When limits are reached, jobs queue. If the queue is full, requests return **429 (Too Many Requests)**—retry with backoff.
</Callout>
...
## Job Triggers at a Glance
- **Manual**: Ad-hoc runs from the UI for debugging and verification.
- **Scheduled**: Cron-based runs for continuous coverage.
- **Remote**: API-triggered for CI/CD and external workflows.
