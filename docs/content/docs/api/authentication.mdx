...
| **Test Token** | `sck_test_` | Full project API access | Reserved programmatic test-token format; the dashboard does not mint these |
...
|----------|---------------|
| **Storage** | Only hashed token material is stored in the database |
| **Lookup** | Exact stored hash lookup; display prefixes are not used for authentication |
| **Comparison** | Stored hash match supports current Better Auth API-key hashes and legacy Supercheck SHA-256 hashes |
...
|------------|-------|--------|
| Job trigger key | Configurable per key; default 100 requests per 60 seconds | Per key |
