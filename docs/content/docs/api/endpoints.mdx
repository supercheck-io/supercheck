...
```bash
curl https://app.supercheck.io/api/health
```
...
```bash
curl https://app.supercheck.io/api/locations
```
...
```bash
curl -N https://app.supercheck.io/api/runs/{runId}/stream \
...
| `testType` | string | `playwright` or `k6` |
...
Supported types: `http`, `ping`, `tcp`, `keyword`
