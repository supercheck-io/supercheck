...
```json
{
  ...
  "data": {
    "jobId": "uuid",
    "jobName": "Nightly E2E Suite",
    "runId": "uuid",
    "testCount": 5,
    "triggeredBy": "CI Trigger",
    "triggeredAt": "2026-02-11T09:47:44.615Z"
  }
}
```
...
```json
{ "message": "Job execution queued successfully.", "jobId": "uuid", "runId": "uuid" }
```
...
| `status` | string | `queued`, `running`, `passed`, `failed`, `error`, `blocked` |
...
```json
{
  ...
  "status": "passed",
  "durationMs": 3000,
  "startedAt": "2026-02-11T09:32:58.363Z",
  "completedAt": "2026-02-11T09:33:02.104Z",
  "logs": null,
  "errorDetails": null,
  "reportUrl": "https://storage.example.com/reports/run-uuid/index.html",
  "trigger": "manual",
  "projectId": "uuid",
  "organizationId": "uuid",
  "testCount": 5
...
Get caller access details for a run, including role and scoped project/organization IDs.
...
| `type` | string | Filter by test type (`browser`, `performance`, `api`, `database`, `custom`) |
...
List all tags currently attached to a test.

### `POST /api/tests/{id}/tags`

Replace a test's tags.
...
List status pages for the current project.

### `GET /api/status-pages/check`

Verify a public status page subdomain and return its status page ID when published.

| Parameter | Type | Description |
|-----------|------|-------------|
| `subdomain` | string | Public status page subdomain |

### `GET /api/status-pages/{id}`

Get status page details including components, incidents, and settings.

### `POST /api/status-pages/{id}/upload`

Upload status page assets (favicon, logo, cover image).

### `GET /api/status-pages/{id}/rss`

Get the public RSS feed for a published status page.
