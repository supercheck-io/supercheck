...
<Callout type="info">
**Tip**: AI Create for browser tests is in beta. For complex flows, consider recording your test first with [Playwright Codegen](https://playwright.dev/docs/codegen) or [Playwright Browser Extension](https://chromewebstore.google.com/detail/playwright-crx/jambeljnbnfbkcpnoiaedcabbgmnnlcd?pli=1), then use AI to enhance it.
</Callout>
...
📚 [Playwright Writing Tests Guide](https://playwright.dev/docs/writing-tests)
</Tab>
<Tab value="API Test">
```typescript
/**
...
📚 [k6 Getting Started](https://grafana.com/docs/k6/latest/) | [k6 Thresholds](https://grafana.com/docs/k6/latest/using-k6/thresholds/)
</Tab>
</Tabs>
...
|----------|-----------------|----------|
| Every hour | `0 * * * *` | Critical paths |
| Every 6 hours | `0 */6 * * *` | Standard monitoring |
| Daily at midnight | `0 0 * * *` | Nightly regression |
...
<Card icon={<Globe className="text-sky-500" />} title="Monitors" href="/docs/monitors">
...
Use [Variables](/docs/automate/variables) to store credentials:
...
For performance tests, select the region when creating the test. Monitors can run from US East, EU Central, and Asia Pacific simultaneously.
