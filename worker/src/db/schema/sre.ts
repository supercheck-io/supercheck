...
type SreDiscoverySuggestionType =
  | 'create_service'
  | 'merge_services'
  | 'add_resource'
  | 'add_dependency';
...
type PrivateAgentJobClass =
  | 'sre_connector_query'
  | 'http_monitor_check'
  | 'playwright_run'
  | 'k6_run'
  | 'network_check';
type PrivateAgentJobStatus =
  | 'queued'
  | 'leased'
  | 'running'
  | 'completed'
...
type ExternalConnectorStatus =
  ...
  | 'missing_credentials'
  | 'disabled';
type ExternalConnectorValidationStatus =
  | 'valid'
  | 'unreachable'
  | 'invalid_credentials'
  | 'policy_blocked';
type ExternalConnectorCredentialType =
  | 'api_key'
  | 'oauth_token'
  | 'bearer_token'
  | 'basic_auth'
...
export const sreServices = pgTable(
  ...
  (table) => ({
    projectOrgStatusIdx: index('sre_services_project_org_status_idx').on(
      table.projectId,
      table.organizationId,
      table.status,
    ),
    activeProjectNameUniqueIdx: uniqueIndex(
      'sre_services_active_project_name_unique_idx',
    )
      .on(table.organizationId, table.projectId, table.name)
      .where(sql`status = 'active'`),
    projectEnvironmentIdx: index('sre_services_project_environment_idx').on(
      table.projectId,
      table.environment,
    ),
    projectTierIdx: index('sre_services_project_tier_idx').on(
      table.projectId,
      table.tier,
    ),
  }),
...
export const sreServiceOwners = pgTable(
  ...
  (table) => ({
    serviceIdIdx: index('sre_service_owners_service_id_idx').on(
      table.serviceId,
    ),
    ownerRefIdx: index('sre_service_owners_owner_type_ref_idx').on(
      table.ownerType,
      table.ownerRef,
    ),
  }),
...
export const sreServiceResources = pgTable(
  'sre_service_resources',
  {
    ...
    relationship: varchar('relationship', { length: 20 })
      .$type<SreResourceRelationship>()
      .notNull()
      .default('monitors'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => ({
    serviceIdIdx: index('sre_service_resources_service_id_idx').on(
      table.serviceId,
    ),
    resourceIdx: index('sre_service_resources_resource_idx').on(
      table.resourceType,
      table.resourceId,
    ),
    serviceResourceUniqueIdx: uniqueIndex(
      'sre_service_resources_service_resource_unique_idx',
    ).on(table.serviceId, table.resourceType, table.resourceId),
  }),
);
...
export const sreServiceDependencies = pgTable(
  'sre_service_dependencies',
  {
    ...
    status: varchar('status', { length: 15 })
      .$type<SreDependencyStatus>()
      .notNull()
      .default('active'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => ({
    activeDependencyUniqueIdx: uniqueIndex(
      'sre_service_dependencies_active_unique_idx',
    )
      .on(table.sourceServiceId, table.targetServiceId)
      .where(sql`status = 'active'`),
    targetServiceIdx: index('sre_service_dependencies_target_service_idx').on(
      table.targetServiceId,
    ),
    projectStatusIdx: index('sre_service_dependencies_project_status_idx').on(
      table.projectId,
      table.status,
    ),
    lastSeenAtIdx: index('sre_service_dependencies_last_seen_at_idx').on(
      table.lastSeenAt,
    ),
    pendingApprovalIdx: index('sre_service_dependencies_pending_approval_idx')
      .on(table.approvedAt)
...
export const sreServiceDependencyObservations = pgTable(
  ...
  (table) => ({
    dependencyIdIdx: index(
      'sre_service_dependency_observations_dependency_id_idx',
    ).on(table.dependencyId),
    edgeIdx: index('sre_service_dependency_observations_edge_idx').on(
      table.sourceServiceId,
      table.targetServiceId,
    ),
    observedAtIdx: index(
      'sre_service_dependency_observations_observed_at_idx',
    ).on(table.observedAt),
  }),
);
...
export const sreServiceHealthSnapshots = pgTable(
  ...
  (table) => ({
    serviceWindowIdx: index(
      'sre_service_health_snapshots_service_window_idx',
    ).on(table.serviceId, table.windowEnd),
    projectHealthIdx: index(
      'sre_service_health_snapshots_project_health_idx',
    ).on(table.projectId, table.health),
    windowEndIdx: index('sre_service_health_snapshots_window_end_idx').on(
      table.windowEnd,
    ),
  }),
);
...
export const sreServiceDiscoverySuggestions = pgTable(
  'sre_service_discovery_suggestions',
  {
    ...
    projectId: uuid('project_id')
      .notNull()
      .references(() => projects.id, { onDelete: 'cascade' }),
    suggestionType: varchar('suggestion_type', { length: 20 })
      .$type<SreDiscoverySuggestionType>()
      .notNull(),
    suggestionData: jsonb('suggestion_data')
      .$type<Record<string, unknown>>()
      .notNull(),
    source: varchar('source', { length: 50 }).notNull(),
    confidence: numeric('confidence', { precision: 5, scale: 4 }),
    status: varchar('status', { length: 15 })
      .$type<SreSuggestionStatus>()
      .notNull()
      .default('pending'),
    approvedBy: uuid('approved_by').references(() => user.id, {
      onDelete: 'set null',
  ...
  (table) => ({
    pendingProjectStatusIdx: index(
      'sre_service_discovery_suggestions_project_pending_idx',
    )
      .on(table.projectId, table.status)
      .where(sql`status = 'pending'`),
    organizationSourceIdx: index(
      'sre_service_discovery_suggestions_org_source_idx',
...
export const sreServiceDeployments = pgTable(
  ...
  (table) => ({
    serviceDeployedAtIdx: index(
      'sre_service_deployments_service_deployed_at_idx',
    ).on(table.serviceId, table.deployedAt),
    projectDeployedAtIdx: index(
      'sre_service_deployments_project_deployed_at_idx',
    ).on(table.projectId, table.deployedAt),
    commitShaIdx: index('sre_service_deployments_commit_sha_idx').on(
      table.commitSha,
    ),
  }),
);
...
export const sreAlertEvents = pgTable(
  'sre_alert_events',
  {
    ...
    serviceId: uuid('service_id').references(() => sreServices.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => ({
    orgFingerprintUniqueIdx: uniqueIndex(
      'sre_alert_events_org_fingerprint_unique_idx',
    ).on(table.organizationId, table.fingerprintHash),
    firingProjectStatusIdx: index('sre_alert_events_project_firing_idx')
      .on(table.projectId, table.status)
      .where(sql`status = 'firing'`),
    serviceStatusIdx: index('sre_alert_events_service_status_idx').on(
      table.serviceId,
      table.status,
    ),
    severityStatusIdx: index('sre_alert_events_severity_status_idx').on(
      table.severity,
      table.status,
    ),
...
export const sreIncidents = pgTable(
  'sre_incidents',
  {
    ...
    status: varchar('status', { length: 25 })
      ...
      .notNull()
      .default('triggered'),
    primaryServiceId: uuid('primary_service_id').references(
      () => sreServices.id,
      { onDelete: 'set null' },
    ),
    statusPageIncidentId: uuid('status_page_incident_id').references(
      () => statusPageIncidents.id,
      { onDelete: 'set null' },
    ),
    triageInvestigationRunId: uuid('triage_investigation_run_id'),
  ...
  (table) => ({
    orgIncidentNumberUniqueIdx: uniqueIndex(
      'sre_incidents_org_incident_number_unique_idx',
    ).on(table.organizationId, table.incidentNumber),
    projectStatusIdx: index('sre_incidents_project_status_idx').on(
      table.projectId,
      table.status,
    ),
    projectSeverityIdx: index('sre_incidents_project_severity_idx').on(
      table.projectId,
      table.severity,
    ),
    primaryServiceStatusIdx: index(
      'sre_incidents_primary_service_status_idx',
    ).on(table.primaryServiceId, table.status),
    activeStatusIdx: index('sre_incidents_active_status_idx')
      .on(table.status)
...
export const sreIncidentAlerts = pgTable(
  'sre_incident_alerts',
  {
    ...
    incidentId: uuid('incident_id')
      .notNull()
      .references(() => sreIncidents.id, { onDelete: 'cascade' }),
    alertEventId: uuid('alert_event_id')
      .notNull()
      .references(() => sreAlertEvents.id, { onDelete: 'cascade' }),
    role: varchar('role', { length: 20 })
      .$type<SreIncidentAlertRole>()
      .notNull()
      .default('trigger'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => ({
    incidentIdIdx: index('sre_incident_alerts_incident_id_idx').on(
      table.incidentId,
    ),
    alertEventIdIdx: index('sre_incident_alerts_alert_event_id_idx').on(
      table.alertEventId,
    ),
    incidentAlertUniqueIdx: uniqueIndex(
      'sre_incident_alerts_incident_alert_unique_idx',
    ).on(table.incidentId, table.alertEventId),
  }),
);
...
export const sreIncidentVerifications = pgTable(
  'sre_incident_verifications',
  {
    ...
    resourceType: varchar('resource_type', { length: 20 })
      .$type<SreVerificationResourceType>()
      .notNull(),
    resourceId: uuid('resource_id').notNull(),
    runId: uuid('run_id'),
    resultStatus: varchar('result_status', { length: 15 })
      .$type<SreVerificationStatus>()
      .notNull()
      .default('pending'),
  ...
  (table) => ({
    incidentIdIdx: index('sre_incident_verifications_incident_id_idx').on(
      table.incidentId,
    ),
...
export const privateAgents = pgTable(
  'private_agents',
  {
    ...
    agentMode: varchar('agent_mode', { length: 20 })
      .$type<PrivateAgentMode>()
      .notNull()
      .default('connector_proxy'),
    supportsSreConnectors: boolean('supports_sre_connectors')
      .notNull()
      .default(true),
    supportsHttpMonitoring: boolean('supports_http_monitoring')
      .notNull()
      .default(false),
    supportsPlaywright: boolean('supports_playwright').notNull().default(false),
    supportsK6: boolean('supports_k6').notNull().default(false),
    supportsNetworkChecks: boolean('supports_network_checks')
      .notNull()
      .default(false),
  ...
  (table) => ({
    organizationStatusIdx: index('private_agents_org_status_idx').on(
      table.organizationId,
      table.status,
    ),
    projectStatusIdx: index('private_agents_project_status_idx').on(
      table.projectId,
      table.status,
    ),
    agentModeStatusIdx: index('private_agents_mode_status_idx').on(
      table.agentMode,
      table.status,
    ),
    connectedHeartbeatIdx: index('private_agents_connected_heartbeat_idx')
...
export const externalConnectors = pgTable(
  'external_connectors',
  {
    ...
    privateAgentId: uuid('private_agent_id').references(
      () => privateAgents.id,
      { onDelete: 'restrict' },
    ),
    name: varchar('name', { length: 100 }).notNull(),
    ...
    riskLevel: varchar('risk_level', { length: 10 })
      .$type<ExternalConnectorRiskLevel>()
      .notNull()
      .default('low'),
    permissionLevel: varchar('permission_level', { length: 10 })
      .$type<ExternalConnectorPermissionLevel>()
      .notNull()
      .default('read'),
    sideEffectLevel: varchar('side_effect_level', { length: 10 })
      .$type<ExternalConnectorSideEffectLevel>()
      .notNull()
      .default('none'),
    status: varchar('status', { length: 20 })
      .$type<ExternalConnectorStatus>()
    ...
    defaultTimeWindowMinutes: integer('default_time_window_minutes')
      .notNull()
      .default(60),
    outputLimits: jsonb('output_limits').$type<Record<string, unknown>>(),
    lastValidatedAt: timestamp('last_validated_at'),
    lastValidationStatus: varchar('last_validation_status', {
      length: 20,
    }).$type<ExternalConnectorValidationStatus>(),
    lastValidationError: text('last_validation_error'),
    lastValidationLatencyMs: integer('last_validation_latency_ms'),
    validatedByPrivateAgentId: uuid('validated_by_private_agent_id').references(
      () => privateAgents.id,
      { onDelete: 'set null' },
    ),
    createdByUserId: uuid('created_by_user_id').references(() => user.id, {
  ...
  (table) => ({
    ...
    organizationTypeIdx: index('external_connectors_org_type_idx').on(
      table.organizationId,
      table.type,
    ),
    validOrganizationStatusIdx: index(
      'external_connectors_valid_org_status_idx',
    )
      .on(table.organizationId, table.status)
      .where(sql`status = 'valid'`),
    projectTypeIdx: index('external_connectors_project_type_idx').on(
      table.projectId,
      table.type,
    ),
    privateAgentIdx: index('external_connectors_private_agent_idx')
      .on(table.privateAgentId)
      .where(sql`private_agent_id IS NOT NULL`),
  }),
);
...
export const privateAgentCredentials = pgTable(
  ...
  {
    id: uuidPk(),
    privateAgentId: uuid('private_agent_id')
      .notNull()
      .references(() => privateAgents.id, { onDelete: 'cascade' }),
    organizationId: uuid('organization_id')
      .notNull()
      .references(() => organization.id, { onDelete: 'cascade' }),
    projectId: uuid('project_id').references(() => projects.id, {
  ...
  (table) => ({
    privateAgentIdIdx: index(
      'private_agent_credentials_private_agent_id_idx',
    ).on(table.privateAgentId),
    keyIdUniqueIdx: uniqueIndex(
      'private_agent_credentials_key_id_unique_idx',
    ).on(table.keyId),
    expiresAtIdx: index('private_agent_credentials_expires_at_idx')
      .on(table.expiresAt)
      .where(sql`expires_at IS NOT NULL`),
    revokedAtIdx: index('private_agent_credentials_revoked_at_idx')
      .on(table.revokedAt)
...
export const privateAgentHeartbeats = pgTable(
  'private_agent_heartbeats',
  {
    ...
    status: varchar('status', { length: 20 })
      .$type<'connected' | 'unhealthy' | 'disconnected'>()
      .notNull(),
    ...
    reportedCapabilities: jsonb('reported_capabilities')
      .$type<Record<string, unknown>>()
      .default({}),
  ...
  (table) => ({
    privateAgentCreatedIdx: index(
      'private_agent_heartbeats_agent_created_idx',
    ).on(table.privateAgentId, table.createdAt),
    organizationCreatedIdx: index(
      'private_agent_heartbeats_org_created_idx',
...
export const externalConnectorServices = pgTable(
  ...
  (table) => ({
    connectorServiceUniqueIdx: uniqueIndex(
      'external_connector_services_connector_service_unique_idx',
    ).on(table.connectorId, table.serviceId),
    projectServiceIdx: index(
      'external_connector_services_project_service_idx',
...
export const externalConnectorCredentials = pgTable(
  'external_connector_credentials',
  {
    ...
    connectorId: uuid('connector_id')
      .notNull()
      .references(() => externalConnectors.id, { onDelete: 'cascade' }),
    credentialType: varchar('credential_type', { length: 20 })
      .$type<ExternalConnectorCredentialType>()
      .notNull(),
  ...
  (table) => ({
    connectorIdIdx: index('external_connector_credentials_connector_id_idx').on(
      table.connectorId,
    ),
    expiresAtIdx: index('external_connector_credentials_expires_at_idx')
      .on(table.expiresAt)
      .where(sql`expires_at IS NOT NULL`),
  }),
);
...
export const diagnosticQueries = pgTable(
  ...
  (table) => ({
    projectConnectorIdx: index('diagnostic_queries_project_connector_idx').on(
      table.projectId,
      table.connectorId,
    ),
    organizationStatusIdx: index('diagnostic_queries_org_status_idx').on(
      table.organizationId,
      table.status,
    ),
    queryTypeStatusIdx: index('diagnostic_queries_query_type_status_idx').on(
      table.queryType,
      table.status,
    ),
  }),
...
export const privateAgentJobs = pgTable(
  'private_agent_jobs',
  {
    ...
    privateAgentId: uuid('private_agent_id')
      .notNull()
      .references(() => privateAgents.id, { onDelete: 'restrict' }),
    connectorId: uuid('connector_id').references(() => externalConnectors.id, {
      onDelete: 'set null',
    }),
    jobClass: varchar('job_class', { length: 30 })
      .$type<PrivateAgentJobClass>()
      .notNull(),
    status: varchar('status', { length: 20 })
      .$type<PrivateAgentJobStatus>()
      .notNull()
      .default('queued'),
    authorizedBy: varchar('authorized_by', { length: 30 })
      .$type<SreActorType>()
      .notNull(),
    ...
    policyDecisionHash: varchar('policy_decision_hash', {
      length: 128,
    }).notNull(),
  ...
  (table) => ({
    privateAgentStatusIdx: index('private_agent_jobs_agent_status_idx').on(
      table.privateAgentId,
      table.status,
    ),
    projectClassStatusIdx: index(
      'private_agent_jobs_project_class_status_idx',
    ).on(table.projectId, table.jobClass, table.status),
    idempotencyKeyUniqueIdx: uniqueIndex(
      'private_agent_jobs_idempotency_key_unique_idx',
    ).on(table.idempotencyKey),
    leaseExpiresAtIdx: index('private_agent_jobs_lease_expires_at_idx')
...
export const privateAgentArtifacts = pgTable(
  ...
  {
    id: uuidPk(),
    privateAgentJobId: uuid('private_agent_job_id')
      .notNull()
      .references(() => privateAgentJobs.id, { onDelete: 'cascade' }),
    organizationId: uuid('organization_id')
      .notNull()
      .references(() => organization.id, { onDelete: 'cascade' }),
    projectId: uuid('project_id')
  ...
  (table) => ({
    privateAgentJobIdIdx: index('private_agent_artifacts_job_id_idx').on(
      table.privateAgentJobId,
    ),
    projectArtifactTypeIdx: index(
      'private_agent_artifacts_project_type_idx',
    ).on(table.projectId, table.artifactType),
    createdAtIdx: index('private_agent_artifacts_created_at_idx').on(
      table.createdAt,
    ),
  }),
);
...
export const sreEvidenceItems = pgTable(
  'sre_evidence_items',
  {
    ...
    sourceType: varchar('source_type', { length: 30 })
      .$type<SreEvidenceSourceType>()
      .notNull(),
    sourceConnectorId: uuid('source_connector_id').references(
      () => externalConnectors.id,
      { onDelete: 'set null' },
    ),
    ...
    evidenceType: varchar('evidence_type', { length: 15 })
      .$type<SreEvidenceType>()
      .notNull(),
    severity: varchar('severity', { length: 10 }),
    confidence: numeric('confidence', { precision: 5, scale: 4 }),
  ...
  (table) => ({
    incidentIdIdx: index('sre_evidence_items_incident_id_idx').on(
      table.incidentId,
    ),
    investigationRunIdIdx: index(
      'sre_evidence_items_investigation_run_id_idx',
    ).on(table.investigationRunId),
    sourceConnectorIdx: index('sre_evidence_items_source_connector_idx').on(
      table.sourceType,
      table.sourceConnectorId,
    ),
    evidenceTypeIdx: index('sre_evidence_items_evidence_type_idx').on(
      table.evidenceType,
    ),
    observedAtIdx: index('sre_evidence_items_observed_at_idx').on(
      table.observedAt,
    ),
  }),
);
...
export const sreInvestigationRuns = pgTable(
  'sre_investigation_runs',
  {
    ...
    alertEventId: uuid('alert_event_id').references(() => sreAlertEvents.id, {
      onDelete: 'set null',
    }),
    agentType: varchar('agent_type', { length: 25 })
      .$type<SreAgentType>()
      .notNull(),
    status: varchar('status', { length: 15 })
      .$type<SreRunStatus>()
    ...
    agentStateSnapshot: jsonb('agent_state_snapshot').$type<
  ...
  (table) => ({
    incidentIdIdx: index('sre_investigation_runs_incident_id_idx').on(
      table.incidentId,
    ),
    projectStatusIdx: index('sre_investigation_runs_project_status_idx').on(
      table.projectId,
      table.status,
    ),
    agentTypeStatusIdx: index(
      'sre_investigation_runs_agent_type_status_idx',
    ).on(table.agentType, table.status),
    modelIdIdx: index('sre_investigation_runs_model_id_idx').on(table.modelId),
    lowConfidenceIdx: index('sre_investigation_runs_low_confidence_idx')
      .on(table.confidenceScore)
      .where(sql`confidence_score < 0.5`),
    createdAtIdx: index('sre_investigation_runs_created_at_idx').on(
      table.createdAt,
    ),
  }),
);
...
export const sreInvestigationToolCalls = pgTable(
  ...
  {
    id: uuidPk(),
    investigationRunId: uuid('investigation_run_id').references(
  ...
  (table) => ({
    ...
    connectorTypeIdx: index(
      'sre_investigation_tool_calls_connector_type_idx',
    ).on(table.connectorType),
    executedAtIdx: index('sre_investigation_tool_calls_executed_at_idx').on(
      table.executedAt,
    ),
  }),
);
...
export const sreAiSuggestions = pgTable(
  'sre_ai_suggestions',
  {
    ...
    suggestionType: varchar('suggestion_type', { length: 30 })
      .$type<SreAiSuggestionType>()
      .notNull(),
  ...
  (table) => ({
    incidentIdIdx: index('sre_ai_suggestions_incident_id_idx').on(
      table.incidentId,
    ),
    suggestionTypeIdx: index('sre_ai_suggestions_type_idx').on(
      table.suggestionType,
    ),
    incidentSuggestionTypeIdx: index('sre_ai_suggestions_incident_type_idx').on(
      table.incidentId,
      table.suggestionType,
    ),
  }),
...
export const sreInvestigationRecommendations = pgTable(
  ...
  (table) => ({
    incidentIdIdx: index(
      'sre_investigation_recommendations_incident_id_idx',
    ).on(table.incidentId),
    pendingIncidentIdx: index(
      'sre_investigation_recommendations_pending_incident_idx',
    )
...
export const sreIncidentTimelineEvents = pgTable(
  'sre_incident_timeline_events',
  {
    ...
    incidentId: uuid('incident_id')
      .notNull()
      .references(() => sreIncidents.id, { onDelete: 'cascade' }),
    eventType: varchar('event_type', { length: 25 })
      .$type<SreTimelineEventType>()
      .notNull(),
    ...
    agentRunId: uuid('agent_run_id').references(() => sreInvestigationRuns.id, {
      onDelete: 'set null',
    }),
    evidenceItemId: uuid('evidence_item_id').references(
      () => sreEvidenceItems.id,
  ...
  (table) => ({
    incidentCreatedIdx: index(
      'sre_incident_timeline_events_incident_created_idx',
    ).on(table.incidentId, table.createdAt),
    actorAgentIdx: index('sre_incident_timeline_events_actor_agent_idx').on(
      table.actorType,
      table.agentRunId,
    ),
    eventTypeIdx: index('sre_incident_timeline_events_event_type_idx').on(
      table.eventType,
    ),
  }),
);
...
export const sreRunbooks = pgTable(
  'sre_runbooks',
  {
    ...
    description: text('description'),
    triggerConditions:
      jsonb('trigger_conditions').$type<Record<string, unknown>>(),
    status: varchar('status', { length: 15 })
      .$type<SreRunbookStatus>()
      .notNull()
      .default('active'),
    createdByUserId: uuid('created_by_user_id').references(() => user.id, {
      onDelete: 'set null',
  ...
  (table) => ({
    projectServiceIdx: index('sre_runbooks_project_service_idx').on(
      table.projectId,
      table.serviceId,
    ),
    organizationNameIdx: index('sre_runbooks_org_name_idx').on(
      table.organizationId,
      table.name,
    ),
  }),
...
export const sreRunbookSteps = pgTable(
  'sre_runbook_steps',
  {
    ...
    runbookId: uuid('runbook_id')
      .notNull()
      .references(() => sreRunbooks.id, { onDelete: 'cascade' }),
    stepOrder: integer('step_order').notNull(),
    instruction: text('instruction').notNull(),
    agentAssisted: boolean('agent_assisted').notNull().default(false),
    evidenceGatheringAction: varchar('evidence_gathering_action', {
      length: 100,
    }),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => ({
    runbookStepOrderIdx: index('sre_runbook_steps_runbook_step_order_idx').on(
      table.runbookId,
      table.stepOrder,
    ),
  }),
...
export const sreIncidentRunbookSteps = pgTable(
  'sre_incident_runbook_steps',
  {
    ...
    incidentId: uuid('incident_id')
      .notNull()
      .references(() => sreIncidents.id, { onDelete: 'cascade' }),
    runbookStepId: uuid('runbook_step_id').references(
      () => sreRunbookSteps.id,
      { onDelete: 'set null' },
    ),
    runbookId: uuid('runbook_id').references(() => sreRunbooks.id, {
      onDelete: 'set null',
    }),
    stepOrder: integer('step_order').notNull(),
    instruction: text('instruction').notNull(),
    status: varchar('status', { length: 15 })
      .$type<SreRunbookStepStatus>()
      .notNull()
      .default('pending'),
  ...
  (table) => ({
    incidentIdIdx: index('sre_incident_runbook_steps_incident_id_idx').on(
      table.incidentId,
    ),
    activeIncidentStatusIdx: index(
      'sre_incident_runbook_steps_active_status_idx',
    )
...
export const sreContextObservations = pgTable(
  ...
  (table) => ({
    serviceIdIdx: index('sre_context_observations_service_id_idx').on(
      table.serviceId,
    ),
    projectObservationTypeIdx: index(
      'sre_context_observations_project_type_idx',
    ).on(table.projectId, table.observationType),
  }),
);
...
export const sreContextRecollections = pgTable(
  ...
  (table) => ({
    incidentIdIdx: index('sre_context_recollections_incident_id_idx').on(
      table.incidentId,
    ),
    serviceIdIdx: index('sre_context_recollections_service_id_idx').on(
      table.serviceId,
    ),
    errorFingerprintIdx: index(
      'sre_context_recollections_error_fingerprint_idx',
    ).on(table.errorFingerprint),
  }),
);
...
export const sreContextPlaybooks = pgTable(
  'sre_context_playbooks',
  {
    ...
    promotedFromRecollectionId: uuid(
      'promoted_from_recollection_id',
    ).references(() => sreContextRecollections.id, { onDelete: 'set null' }),
    name: varchar('name', { length: 200 }).notNull(),
    alertSignatureHash: varchar('alert_signature_hash', {
      length: 64,
    }).notNull(),
    alertSignature: jsonb('alert_signature')
      .$type<Record<string, unknown>>()
      .notNull(),
    playbookContent: text('playbook_content').notNull(),
    matchCount: integer('match_count').notNull().default(0),
    status: varchar('status', { length: 15 })
      .$type<SreContextPlaybookStatus>()
      .notNull()
      .default('active'),
    createdByUserId: uuid('created_by_user_id').references(() => user.id, {
      onDelete: 'set null',
  ...
  (table) => ({
    organizationAlertSignatureIdx: index(
      'sre_context_playbooks_org_alert_signature_idx',
    ).on(table.organizationId, table.alertSignatureHash),
    projectStatusIdx: index('sre_context_playbooks_project_status_idx').on(
      table.projectId,
      table.status,
    ),
    alertSignatureGinIdx: index(
      'sre_context_playbooks_alert_signature_gin_idx',
    ).using('gin', table.alertSignature),
  }),
);
...
export const sreBackgroundAgentRuns = pgTable(
  ...
  (table) => ({
    activeProjectStatusIdx: index(
      'sre_background_agent_runs_active_project_status_idx',
    )
      .on(table.projectId, table.status)
      .where(sql`status IN ('scheduled', 'running')`),
    scheduleCreatedIdx: index(
      'sre_background_agent_runs_schedule_created_idx',
...
export const sreChatConversations = pgTable(
  'sre_chat_conversations',
  {
    ...
    status: varchar('status', { length: 15 })
      .$type<SreChatConversationStatus>()
      .notNull()
      .default('active'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
  ...
  (table) => ({
    userUpdatedAtIdx: index('sre_chat_conversations_user_updated_at_idx').on(
      table.userId,
      table.updatedAt,
    ),
    incidentIdIdx: index('sre_chat_conversations_incident_id_idx').on(
      table.incidentId,
    ),
    projectStatusIdx: index('sre_chat_conversations_project_status_idx').on(
      table.projectId,
      table.status,
    ),
  }),
...
export const sreChatMessages = pgTable(
  ...
  (table) => ({
    conversationCreatedIdx: index(
      'sre_chat_messages_conversation_created_idx',
...
export const sreInsertSchemas = Object.fromEntries(
  Object.entries(sreTables).map(([name, table]) => [
    name,
...
export type SreInvestigationRunInsert =
  typeof sreInvestigationRuns.$inferInsert;
export type SreInvestigationToolCall =
  typeof sreInvestigationToolCalls.$inferSelect;
export type SreInvestigationToolCallInsert =
  typeof sreInvestigationToolCalls.$inferInsert;
...
export type SreChatConversationInsert =
