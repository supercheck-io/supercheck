...
export class K6ExecutionService {
  ...
  /**
   * Categorize K6 error based on exit code and output
   * Returns structured error information for better error handling
   */
  private categorizeK6Error(
    exitCode: number,
    stderr: string,
    stdout: string,
  ): {
    ...
    message: string;
    details: string;
    category: 'syntax' | 'runtime' | 'threshold' | 'config' | 'unknown';
  } {
    const stderrLower = stderr.toLowerCase();
    const stdoutLower = stdout.toLowerCase();

    // Exit code 255: Script syntax/parse errors
    if (exitCode === 255) {
      // Extract the actual error message from stderr
      const syntaxErrorMatch = stderr.match(
        /level=error msg="([^"]+)"/i,
      );
      const errorLocation = stderr.match(/test\.js:(\d+:\d+)/i);

      let errorMessage = 'Script syntax or parsing error';
      if (syntaxErrorMatch) {
      ...
      if (errorLocation) {
        errorMessage += ` at line ${errorLocation[1]}`;
      }
    ...
    // Exit codes 97-99: K6 runtime errors
    if (exitCode >= 97 && exitCode <= 99) {
      // Extract runtime error from stderr
      const runtimeErrorMatch = stderr.match(
        /ERRO(?:R)?\[[\d]+\] (.+?)(?:\n|$)/i,
      );

      return {
        isScriptError: true,
        message: runtimeErrorMatch
          ? runtimeErrorMatch[1]
          : 'K6 runtime error during test execution',
    ...
    if (
      ...
      stderrLower.includes('import') && stderrLower.includes('error')
    ) {
      const moduleErrorMatch = stderr.match(
        /cannot find module ["']([^"']+)["']/i,
      );

      return {
        isScriptError: true,
        message: moduleErrorMatch
          ? `Cannot find module '${moduleErrorMatch[1]}'`
          : 'Module import error',
        details: stderr.trim(),
        category: 'runtime',
      };
    ...
    if (
      stderrLower.includes('invalid config') ||
      stderrLower.includes('invalid option') ||
      stderrLower.includes('invalid argument')
    ) {
      return {
        isScriptError: true,
        message: 'K6 configuration error',
        details: stderr.trim() || stdout.trim(),
        category: 'config',
      };
    }

    // Exit code 99 or threshold failures in output
    if (
      exitCode === 99 ||
      stdoutLower.includes('thresholds') && stdoutLower.includes('failed')
    ) {
      return {
        isScriptError: false, // This is expected test failure, not a script error
        message: 'K6 thresholds failed',
        details: stdout.trim(),
        category: 'threshold',
      };
    }
    ...
    return {
      isScriptError: exitCode >= 100, // High exit codes are usually script errors
      message: `K6 exited with code ${exitCode}`,
      details: (stderr.trim() || stdout.trim()).substring(0, 1000),
      category: 'unknown',
    };
  }
  ...
  async runK6Test(task: K6ExecutionTask): Promise<K6ExecutionResult> {
    ...
    try {
      ...
      // Check for k6 script errors BEFORE checking for HTML report
      // Exit code 255 typically indicates script syntax/parse errors
      // Exit codes 97-99 indicate runtime errors
      if (!timedOut && execResult.exitCode !== 0) {
        const k6Error = this.categorizeK6Error(
          execResult.exitCode,
          execResult.stderr,
          execResult.stdout,
        );

        // If this is a script error (syntax, parse, etc.), throw immediately with the actual error
        if (k6Error.isScriptError) {
          this.logger.error(
            `[${runId}] K6 script error detected: ${k6Error.message}`,
          );
          throw new Error(
            `K6 script error: ${k6Error.message}\n\nDetails:\n${k6Error.details}`,
          );
        }
      }
      ...
      try {
        ...
        if (timedOut) {
          ...
          // Only log as warning if k6 executed successfully but report is missing
          // This indicates a dashboard configuration issue, not a script error
          const severity = execResult.exitCode === 0 ? 'error' : 'warn';
          this.logger[severity](
            `[${runId}] HTML report not generated by k6 web-dashboard at ${htmlReportPath}: ${getErrorMessage(error)}`,
          );

          // Only throw if k6 executed successfully (exit code 0)
          // If k6 failed for other reasons, we'll handle it via the thresholds check
   ...
   *
   * IMPORTANT: This prepends trace context info as comments/variables without
   * modifying the user script structure to avoid introducing syntax errors.
   */
  private wrapScriptWithTraceContext(userScript: string): string {
    ...
    // Instead of wrapping and re-exporting (which can break scripts),
    // we prepend trace context helpers that users can optionally use.
    // This approach doesn't modify the script structure at all.
    const traceContextHelper = `
// ==== SuperCheck Trace Context Propagation ====
// Trace context is available via environment variables for end-to-end observability
//
// Available environment variables:
//   - __ENV.TRACEPARENT: W3C Trace Context header value
//   - __ENV.OTEL_TRACE_ID: OpenTelemetry Trace ID
//   - __ENV.OTEL_SPAN_ID: OpenTelemetry Span ID
//
// To enable trace propagation in your K6 script, add the traceparent header to your requests:
//
// Example with k6 http module:
//   import http from 'k6/http';
//
//   const params = {
//     headers: {
//       'traceparent': __ENV.TRACEPARENT || '',
//       'Content-Type': 'application/json',
//     }
//   };
//   http.get('https://api.example.com/endpoint', params);
//
// Helper function to add trace headers to request params
function withTraceHeaders(params) {
...
`;
    ...
    // Simply prepend the helper - don't wrap or modify the user script
    return traceContextHelper + userScript;
  }
