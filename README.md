...
flowchart TB
    ...
    T --> App[Next.js App<br/>UI + API + AI SRE Engine]
    App --> DB[(PostgreSQL<br/>Primary DB)] & Redis[(Redis + BullMQ<br/>Queue + Cache)] & S3[(MinIO<br/>Artifacts)]
    App -->|read-only queries| Direct[Direct Connectors<br/>Prometheus · Loki · Tempo · K8s]
    App -.->|job lease| Private[Private Agents<br/>Outbound HTTPS only]
    ...
    subgraph PRIMARY["Primary Server"]
        ...
        App -.-> SRE_WS[AI SRE Agent Workspace<br/>gVisor-sandboxed pod]
    end
    ...
    style Direct fill:#0ea5e9,stroke:#0369a1,color:#fff
    style Private fill:#8b5cf6,stroke:#6d28d9,color:#fff
    style SRE_WS fill:#059669,stroke:#047857,color:#fff
